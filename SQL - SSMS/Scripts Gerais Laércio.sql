...
--EXEC Rodomaior_Gse.dbo.ListarDFePorDemanda_Pendente
...
--SELECT Tipo, Sequencial, Filial, TipoCTe, UF, NumConhecto, DataEmissao, Cliente, Funcionario, Valor, Status, CodStatus, DataHoraAutorizacao, TempoEspera, Ambiente, TipoCTe, Servidor FROM Rodomaior_Gse.dbo.ConsultaCTeNova WHERE DataEmissao BETWEEN GETDATE () AND GETDATE () AND (CodStatus NOT IN (1, 4, 5, 9) OR (DataHoraAutorizacao IS NULL AND CodStatus NOT IN (5,9))) ORDER BY DataEmissao DESC
