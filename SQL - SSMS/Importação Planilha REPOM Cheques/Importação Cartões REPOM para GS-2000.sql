...
FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','Excel 8.0;DATABASE=C:\EDI\Cartões REPOM RODOMAIOR 06-06-2024.xlsx','SELECT * FROM [Planilha1$A1:E10000]') P
...
select 'CG', 105    --> Conta da REPOM na Rodomaior
     , 5          --> Sequencial do Talão cadastrado na etapa 3.
     , P.Codigo_Envelope, P.Codigo_Cartao, P.ID_Lote_Cartao_Embossing, P.Data_Vencimento --> campos da planilha da REPOM
FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','Excel 8.0;DATABASE=C:\EDI\Cartões REPOM RODOMAIOR 06-06-2024.xlsx','SELECT * FROM [Planilha1$A1:E10000]') P
where p.Codigo_Envelope is not null
