...
SELECT CodRota,CodPedidoFrete, CodOrdemEmbarque, * from ConhecimentosTransporte where NumConhecto = 15030    and CodUF = 'RO' -- antigo cod rota 510410
...
--
select * from Lontano_GSe.dbo.Descarga_MDFe where Sequencial = 1752788	 --and SeqDescarga = 1 -- para saber qual está (DESCARGA)
select CodUFIBGE,* from Lontano_GSe.dbo.Cidades_e where NomeMunicipioIBGE like '%Crespo%'  -- colocar o destino de descarga
select * FROM Lontano_GSe.dbo.Cidades_e where CodMunicipioIBGE =  5107925
--update Lontano_GSe.dbo.Descarga_MDFe set cMunDescarga = 1100262, xMunDescarga = 'Rio Crespo' where Sequencial = 1752788	
--
...
--update Lontano_GSe.dbo.MDFe set status = 0 where Sequencial = 1100262
--
...
DECLARE @VCOMP AS NUMERIC SET @VCOMP  = (select  vComp from Lontano_GSe.dbo.CompPagamentoFrete_MDFe where Sequencial = 1730110)
DECLARE @VCONTRATO AS NUMERIC SET @VCONTRATO = (select vContrato  from Lontano_GSe.dbo.PagamentoFrete_MDFe where Sequencial = 1730110)
--
...
INNER JOIN Lontano_GSe.dbo.PagamentoFrete_MDFe PF ON PP.Sequencial = PF.Sequencial where PP.Sequencial = 1730110
DECLARE @SOMATORIO AS NUMERIC SET @SOMATORIO = (select vAdiant + vParcela AS SOMATORIO from Lontano_GSe.dbo.ParcelasPagamentoFrete_MDFe PP INNER JOIN Lontano_GSe.dbo.PagamentoFrete_MDFe PF ON PP.Sequencial = PF.Sequencial where PP.Sequencial = 1730110)
SELECT @SOMATORIO AS SOMATORIO, @VCOMP AS VCOMP, @VCONTRATO AS VCONTRATO

--update Lontano_GSe.dbo.CompPagamentoFrete_MDFe set vComp = 18262.01 where Sequencial = 1730114
--update Lontano_GSe.dbo.PagamentoFrete_MDFe set vContrato = 18262.01 where Sequencial = 1730114
