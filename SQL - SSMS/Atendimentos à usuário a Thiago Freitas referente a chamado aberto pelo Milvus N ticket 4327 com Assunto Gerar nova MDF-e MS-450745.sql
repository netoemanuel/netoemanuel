...
SELECT CodRota,CodPedidoFrete,CodVeiculo, CodOrdemEmbarque, * from ConhecimentosTransporte where NumConhecto = 669413   and CodUF = 'MT' -- antigo cod rota 39372
--UPDATE ConhecimentosTransporte SET CodRota = 85752  WHERE Sequencial = 120000022616
SELECT CodRota, * from PedidosFrete where CodPedidoFrete = 12001197
--UPDATE PedidosFrete SET CodRota = 85752  WHERE CodPedidoFrete = 12001197
...
EXEC MonitoraMDFe 12036947

-- Caso dê a Rejeição "612"	Necessario Trocar o	Codigo do Municipio de Descarga e o nome do municipio
select * from Lontano_GSe.dbo.Descarga_MDFe where Sequencial = 1449156 --and SeqDescarga = 1 -- para saber qual está (DESCARGA)
select CodUFIBGE,* from Lontano_GSe.dbo.Cidades_e where NomeMunicipioIBGE like '%Porto%Franco%'  -- colocar o destino de descarga
select * FROM Lontano_GSe.dbo.Cidades_e where CodMunicipioIBGE =  2109007
--update Lontano_GSe.dbo.Descarga_MDFe set cMunDescarga = 2109007, xMunDescarga = 'Porto Franco' where Sequencial = 1449156	and SeqDescarga = 1 

--DELETE FROM Lontano_GSe.dbo.MDFe where Sequencial = 1356407

--Reenviar para autorizar	
EXEC MonitoraMdfe 12036947	
SELECT STATUS, * FROM Lontano_GSe.dbo.MDFe WHERE Sequencial = 1449156
--update Lontano_GSe.dbo.MDFe set status = 0 where Sequencial = 1449156
...
DECLARE @VCOMP AS NUMERIC SET @VCOMP  = (select  vComp from Lontano_gse.dbo.CompPagamentoFrete_MDFe where Sequencial = 468504)
DECLARE @VCONTRATO AS NUMERIC SET @VCONTRATO = (select vContrato from Lontano_gse.dbo.PagamentoFrete_MDFe where Sequencial = 468504)
...
INNER JOIN Lontano_gse.dbo.PagamentoFrete_MDFe PF ON PP.Sequencial = PF.Sequencial where PP.Sequencial = 468504
DECLARE @SOMATORIO AS NUMERIC SET @SOMATORIO = (select vAdiant + vParcela AS SOMATORIO from Lontano_gse.dbo.ParcelasPagamentoFrete_MDFe PP INNER JOIN Lontano_gse.dbo.PagamentoFrete_MDFe PF ON PP.Sequencial = PF.Sequencial where PP.Sequencial = 468504)
SELECT @SOMATORIO AS SOMATORIO, @VCOMP AS VCOMP, @VCONTRATO AS VCONTRATO

update Lontano_gse.dbo.CompPagamentoFrete_MDFe set vComp = 5777.44 where Sequencial = 468504
update Lontano_gse.dbo.PagamentoFrete_MDFe set vContrato = 5777.44 where Sequencial = 468504
